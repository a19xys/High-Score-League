...
test("renderer only signals network changes and never confirms connected", async () => {
  ...
  assert.match(app, /openRankingWithOperationFeedback/);
  assert.match(app, /busyLabel: "Abriendo ranking"/);
});
...
test("header keeps a binary CSS status and integrates refresh in the same chip", async () => {
  ...
  assert.match(styles, /\.connection-chip\s*\{[\s\S]*?gap: 6px;[\s\S]*?inline-size: max-content;[\s\S]*?max-inline-size: 100%/);
  ...
  assert.match(styles, /\.connection-refresh-button\s*\{[\s\S]*?margin-inline-start: 2px[\s\S]*?border: 0[\s\S]*?background: transparent[\s\S]*?color: inherit/);
...
test("header selector exposes semantic states without changing remote authority", async () => {
  ...
  assert.equal(deriveConnectivityHeaderState(null), "disconnected");
  assert.equal(deriveConnectivityHeaderState(undefined), "disconnected");
  assert.equal(deriveConnectivityHeaderState({}), "disconnected");
  assert.equal(deriveConnectivityHeaderState({ reachability: "unknown" }), "disconnected");
  ...
  assert.equal(deriveConnectivityHeaderState({ reachability: "connected" }, { status: "missing" }), "disconnected");
  assert.equal(deriveConnectivityHeaderState({ reachability: "connected" }, { status: "invalid" }), "disconnected");
});

test("first header render reserves connectivity without presenting a false negative", async () => {
  ...
  assert.match(header, /data-connectivity-committed="false"/);
  assert.match(header, /connection-chip--unresolved"[\s\S]*aria-hidden="true"/);
  assert.doesNotMatch(header, /data-connectivity-status=/);
  assert.doesNotMatch(header, /connection-dot|>Desconectado<|>Conectado<|Comprobando conexión|Reconectando/);
  assert.match(header, /connection-refresh-button[\s\S]*aria-hidden="true" tabindex="-1"/);
});

test("remote configuration problems project to disconnected in the normal header", async () => {
  const { renderHeader } = await import(
    pathToFileURL(path.join(rendererRoot, "components", "header.js")).href
  );
  const header = renderHeader({
  ...
  assert.match(header, /data-connectivity-status="disconnected"/);
  assert.match(header, /Desconectado/);
  assert.doesNotMatch(header, /Configuración HSL inválida|data-remote-configuration-status/);
});
...
test("header and Ranking render the same committed gate for loss and recovery", async () => {
  ...
  assert.match(renderHeader(offline), /data-connectivity-status="disconnected"/);
