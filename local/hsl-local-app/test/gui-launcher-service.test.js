...
test("renderer product hierarchy includes connection, player actions, activity and advanced options", async () => {
  ...
  assert.match(app, /createRegionRenderer/);
  assert.match(app, /function captureRegionInteraction\(region\)/);
  assert.match(app, /function restoreRegionInteraction\(region, interaction\)/);
  assert.match(app, /function detailScrollKeyFromState\(state\)/);
  assert.match(app, /nextGameStructureKey !== currentGameStructureKey/);
  assert.match(app, /nextDetailScrollKey !== currentDetailScrollKey/);
  ...
  assert.match(gamePanel, /function deriveGameHeroStatusPresentation\(state\)/);
  assert.match(gamePanel, /game-hero-indicator--status/);
  assert.match(gamePanel, /renderIcon\("star-filled"/);
  assert.match(gamePanel, /icon: "check"/);
  assert.match(gamePanel, /icon: "refresh"/);
  ...
  assert.match(styles, /\.pack-card--covers \.pack-card__media[\s\S]*aspect-ratio: 2 \/ 3/);
  assert.match(app, /Launcher listo/);
  assert.doesNotMatch(app, /Launcher actualizado/);
