...
test("renderer expone resumen visual y checks técnicos sin secretos", async () => {
  const [gamePanel, devTools, presentation] = await Promise.all([
    ...
    fsp.readFile(path.join(__dirname, "..", "gui", "renderer", "product-presentation.js"), "utf8"),
  ]);

  assert.match(gamePanel, /deriveGameSummaryPresentation/);
  assert.match(presentation, /Pack listo/);
  assert.match(presentation, /Pack listo con avisos/);
  assert.match(presentation, /readiness\.canPractice/);
  assert.match(presentation, /readiness\.canPlayCompetition/);
