...
`Jugar` y los botones `primary` de `app-dialog` son las dos familias realmente full-fill y comparten un único contrato de custom properties/selector. Conservan su superficie propia en reposo; hover modifica moderadamente fondo, borde, halo y profundidad, `focus-visible` dibuja un ring inequívoco y `active` desplaza un píxel sin alterar layout. Disabled no recibe hover ni active. Las transiciones enumeran propiedades concretas y no usan `transition: all` ni `filter`. `account-primary` sigue siendo una superficie tintada y no adopta esta jerarquía.
...
La capa `.modal-layer`, que ya comienza bajo la titlebar nativa, es también la frontera de pintura mediante `overflow: hidden`. De este modo el blur vertical de la sombra del drawer no puede pintar sobre la barra, mientras su proyección hacia la izquierda sigue visible dentro de la capa y el scroll permanece en el cuerpo interno del drawer.
