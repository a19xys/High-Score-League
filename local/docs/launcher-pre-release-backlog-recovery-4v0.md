...
| Solucionado documentalmente | 7 |
| Requiere contraste técnico posterior | 3 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PR-001 Feedback de acción Ranking | La pulsación explícita debe mostrar comprobando/abriendo/error, impedir doble apertura y cerrar feedback sin alterar conectividad; el batch automático sigue silencioso. | Conversación A; `operation-feedback-lifecycle-1.md`, “Scopes” y cierre; `ranking-session-verification-1.md`, triggers. Externa posterior a 3B.3. | Petición explícita recuperada | antes de RC · pendiente · alta | 3B.3 + gate Ranking; botón, overlay y live region | Alto si se usa busy global o se toca health · tarea Ranking interactivo | Feedback inmediato solo para intención del usuario; runId; error/cierre; sin doble apertura ni falso estado global. | conversación + apoyo documental |
...
| PR-006 Estado inicial Space Invaders v2 | Confirmar que no aparece indefinidamente ni transita por badges duplicados `Listo con avisos`/`Auto-sync activo` antes del estado estable. | Conversación G; `launcher-startup-theme-assets-3b2.md`; `launcher-product-state-presentation-3b3.md`, badges/cola; cierre sin ventana visual. | QA o validación pendiente | antes de RC · pendiente QA · alta | 3B.2/3B.3; startup, detalle, cola | Alto si se parchea con delay o copy local · QA de estado real | Recorrido real documentado: un resumen priorizado, sin falso listo, sin duplicados y sin flicker persistente. | conversación + documentación |
...
| SOL-006 Menú de cuenta final | Header por siglas, emails visibles en menú, olvidar por fila y login efímero son la cadena vigente. | account menu polish→behavior→compact→layout; 3B.3 login. | Solucionado documentalmente | antes de RC · solucionado · alta | 3B.1/3B.3 + cuentas | Alto si se recupera copy/botón obsoleto · guardrail | Mantener foco, apertura limpia y borrado seguro; no reintroducir botón global ni metadata como auth. | documentación |
...
| CON-003 Punto o SVG para conectividad | Icon visual polish declara punto+texto sin SVG; icon system enumera SVG de estado; 3B.3 exige icono/texto/borde semánticos sin fijar asset. | `icon-visual-polish-2.md`, “Aplicado”; `icon-system-1.md`, usos; 3B.3 severidades. | Requiere contraste técnico posterior | dentro de RC · requiere contraste · media | 3B.3; header/status primitives | Bajo/medio si se duplica iconografía · polish de header solo si hay defecto | Confirmar presentación vigente; mantener texto y no depender del color; una única primitiva. | documentación |
