...
| I. Favorito y `Pack listo` en el hero | PR-012 | 4V.3 retira la estrella especial del título y sitúa Favorito con estrella y `Pack listo` con check en una región responsive independiente del hero. |
...
| PR-012 Favorito y `Pack listo` en el hero | La estrella especial y su medición desaparecen del título. Una región hermana de assets muestra Favorito con `star-filled` y `Pack listo` con check abajo a la derecha: píldoras con espacio y círculos de solo icono en ancho compacto. | Fuente: petición del usuario recuperada de la conversación, corregida expresamente de corazón a estrella; implementación 4V.3 en renderer regional. | Petición explícita implementada | antes de RC · implementado en 4V.3; QA visual física pendiente · alta | 3B.2/3B.3; hero, assets y resumen canónico de producto | Mitigado: indicadores regionales, container queries sin medición JS y supresión del duplicado inferior | Favorito solo con `game.favorite`; `Pack listo` solo con `competition-ready`; estrella/check accesibles, sin corazón, sin recargar hero/logo ni cambiar persistencia de favoritos. | conversación + implementación 4V.3 |
...
2. PR-012, composición de favorito y `Pack listo` en hero implementada en 4V.3;
   queda QA visual física.
