...
| I. Favorito y estado del pack en el hero | PR-012 | 4V.3A conserva Favorito con estrella y unifica `Comprobando`, `Pack listo` y `Con errores` en un carril del hero separado de sus assets y protegido frente al logo. |
...
| PR-012 Favorito y estado del pack en el hero | La estrella especial y su medición desaparecen del título. Un Grid separa logo y carril de indicadores: Favorito conserva `star-filled`; el estado evoluciona entre `Comprobando`, `Pack listo` y `Con errores`, con píldoras o círculos según el espacio real del carril. | Fuente: petición del usuario recuperada de la conversación, corregida de corazón a estrella; implementaciones 4V.3 y 4V.3A en renderer regional. | Petición explícita implementada | antes de RC · corregido en 4V.3A; QA con packs físicos pendiente · alta | 3B.2/3B.3; hero, assets y resumen canónico de producto | Mitigado: columnas estructurales anti-colisión, container queries sobre el carril, proyección canónica con precedencia error/checking/ready y estilos claros específicos | Favorito independiente; estado genérico sin revelar errores concretos; sin duplicados bajo el hero, sin corazón y sin recargar hero/logo ni cambiar persistencia de favoritos. | conversación + implementación 4V.3A |
...
2. PR-012, composición de favorito y estado del pack corregida en 4V.3A con
   carril anti-colisión; queda QA con packs físicos.
...
3. **4V.3–4V.3A — Hero.** PR-012: favorito y estado canónico adaptables en un
   carril inferior derecho que no invade el logo, sin tocar persistencia.
